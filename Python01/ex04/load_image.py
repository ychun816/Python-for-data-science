...
def ft_load(path: str) -> np.ndarray:
    ...
    Returns an RGB NumPy array on success or an error string on failure
    (preserves original behaviour). The function prints basic format and
    shape information when successful.
    """
    try:
...
def main() -> int:
    """Demo runner for the ex04 image loader.

    This function is guarded and returns an exit code. It avoids
    uncaught exceptions during import and provides a small local demo.
    """
    try:
        candidates = [
            "happydoggie.jpg",

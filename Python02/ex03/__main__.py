...
Usage examples (copy-paste to test):
    ...
    # save to a PNG (headless)
    python __main__.py 2019 --save ./ex03_2019_cli.png

    # short form
    python __main__.py 2019 -s ex03_2019.png

The `--save` / `-s` flag saves the current figure to the given path
instead of (or in addition to) attempting to open a GUI window.

Note: headless containers typically lack a DISPLAY, so `plt.show()` will
not open a window. Use `--save` when running inside a container.
"""
...
def main(argv):
    parser = argparse.ArgumentParser(
        description="Plot GDP vs Life Expectancy for a year",
    )
    ...
    parser.add_argument(
        ...
        "--save",
        dest="save",
        help="Path to save PNG instead of showing",
    )
